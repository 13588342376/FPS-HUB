...
function switchSandboxRole(role, btnElement) {
    ...
    if (role === 'fragger') {
        quoteEl.style.opacity = "0"; setTimeout(function(){ quoteEl.innerText = "「一马当先，撕裂防线！寻找能跟上我补枪节奏的突击兄弟！」"; quoteEl.style.opacity = "1"; }, 200);
        quoteEl.style.color = "#ff4b2b";
        quoteEl.style.textShadow = "0 0 10px rgba(255,75,43,0.3)";
    } else if (role === 'awper') {
        quoteEl.style.opacity = "0"; setTimeout(function(){ quoteEl.innerText = "「长线架枪已就绪，A大鸟都飞不过去。帮我防好屁股就行。」"; quoteEl.style.opacity = "1"; }, 200);
        quoteEl.style.color = "#ffd700";
        quoteEl.style.textShadow = "0 0 10px rgba(255,215,0,0.3)";
    } else if (role === 'igl') {
        quoteEl.style.opacity = "0"; setTimeout(function(){ quoteEl.innerText = "「Rush B 只是幌子，听我战术手势，第二时间静步控图，稳稳拿下！」"; quoteEl.style.opacity = "1"; }, 200);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (joinBtn) {
        joinBtn.addEventListener('click', function(e) {
            // 🚨 核心：阻止 <a> 标签默认的 # 锚点置顶行为，由我们自己控制滚动
            e.preventDefault(); 
            
            // 1. 判断是否已登录：检查导航栏状态文本是否包含“特工”，或者 body 是否有登录类名
            const navStatus = document.querySelector('.nav-user-status');
            const isLoggedIn = document.body.classList.contains('logged-in') || 
                             (navStatus && navStatus.innerText.includes('特工'));

            if (isLoggedIn) {
                // ✨ 情况 A：已经是登录特工！丝滑平滑滚动到最底部的【加入我们】板块
                const joinSection = document.querySelector('.home-join-wild');
                if (joinSection) {
                    joinSection.scrollIntoView({ behavior: 'smooth' });
                } else {
                    // 备用方案：如果找不到对应的 class，就根据你底部的 id 或标签向下滚动
                    window.scrollTo({
